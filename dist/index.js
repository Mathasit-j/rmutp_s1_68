...
app.post("/profile", async (c) => {
    ...
    if (existingProfile) {
        ...
        if (decrypt(existingProfile.mobile) === body.mobile)
            duplicatedFields.push('mobile');
        if (decrypt(existingProfile.cardId) === body.cardId)
    ...
    // decode ก่อนส่งกลับ
    const output = Object.assign(Object.assign({}, result), { mobile: decrypt(result.mobile), cardId: decrypt(result.cardId) });
    // output
    c.status(200);
    return c.json({
